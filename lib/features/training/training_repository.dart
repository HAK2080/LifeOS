...
class TrainingRepository {
  ...
  Stream<List<Exercise>> watchExercises() =>
      (db.select(db.exercises)..orderBy([(t) => OrderingTerm.asc(t.name)]))
          .watch();

  Future<int> addCustomExercise(String name,
  ...
  Stream<WorkoutSession?> watchActiveSession() => (db.select(db.workoutSessions)
        ..where((t) => t.finishedAt.isNull())
        ..orderBy([(t) => OrderingTerm.desc(t.startedAt)])
        ..limit(1))
      .watchSingleOrNull();

  Future<int> startSession({int? planWorkoutId, String title = 'Workout'}) =>
  ...
  Future<void> finishSession(int sessionId, {String? notes}) async {
    ...
    for (final link in links) {
      final sets = await (db.select(db.sessionSets)
        ...
        .get();
      if (sets.isEmpty) {
        await (db.delete(db.sessionSets)
              ..where((t) => t.sessionExerciseId.equals(link.id)))
            .go();
        await (db.delete(db.sessionExercises)..where((t) => t.id.equals(link.id)))
            .go();
      } else {
  ...
  Future<int> addExerciseToSession(int sessionId, int exerciseId) async {
    final count = await (db.select(db.sessionExercises)
          ..where((t) => t.sessionId.equals(sessionId)))
        .get();
    final linkId = await db.into(db.sessionExercises).insert(
  ...
  Future<void> removeSessionExercise(int linkId) async {
    await (db.delete(db.sessionSets)
        ...
        .go();
    await (db.delete(db.sessionExercises)..where((t) => t.id.equals(linkId)))
        .go();
  }
  ...
  Future<int> addSet(int linkId) async {
    ...
    return db.into(db.sessionSets).insert(SessionSetsCompanion.insert(
          sessionExerciseId: linkId,
  ...
  Future<void> updateSet(int setId, SessionSetsCompanion changes) =>
      (db.update(db.sessionSets)..where((t) => t.id.equals(setId)))
          .write(changes);
  ...
  Future<LastPerformance?> lastPerformance(int exerciseId) async {
    final rows = await (db.select(db.sessionExercises).join([
...
extension PlanQueries on TrainingRepository {
  Stream<List<WorkoutPlan>> watchPlans() => (db.select(db.workoutPlans)
  ...
  Future<void> archivePlan(int planId) =>
      (db.update(db.workoutPlans)..where((t) => t.id.equals(planId)))
          .write(const WorkoutPlansCompanion(archived: Value(true)));
  ...
  Future<int> addPlanWorkout(int planId, String name) async {
    final existing = await (db.select(db.planWorkouts)
          ..where((t) => t.planId.equals(planId)))
        .get();
    return db.into(db.planWorkouts).insert(PlanWorkoutsCompanion.insert(
        planId: planId, name: name, position: existing.length));
  }
  ...
  Future<void> deletePlanWorkout(int workoutId) async {
    await (db.delete(db.planExercises)
        ...
        .go();
    await (db.delete(db.planWorkouts)..where((t) => t.id.equals(workoutId)))
        .go();
  }

  Stream<List<(PlanExercise, Exercise)>> watchPlanExercises(int workoutId) {
    final q = (db.select(db.planExercises)
          ..where((t) => t.planWorkoutId.equals(workoutId))
          ..orderBy([(t) => OrderingTerm.asc(t.position)]))
        .join([
      innerJoin(
          db.exercises, db.exercises.id.equalsExp(db.planExercises.exerciseId)),
    ]);
    return q.watch().map((rows) => rows
        .map((r) => (r.readTable(db.planExercises), r.readTable(db.exercises)))
        .toList());
  }

  Future<int> addPlanExercise(int workoutId, int exerciseId,
      {int targetSets = 3, int? repMin, int? repMax}) async {
    final existing = await (db.select(db.planExercises)
          ..where((t) => t.planWorkoutId.equals(workoutId)))
        .get();
    return db.into(db.planExercises).insert(PlanExercisesCompanion.insert(
  ...
  Future<void> updatePlanExercise(int id, PlanExercisesCompanion changes) =>
      (db.update(db.planExercises)..where((t) => t.id.equals(id)))
          .write(changes);
  ...
  Future<PlanWorkout?> nextPlanWorkout(int planId) async {
    final workouts = await (db.select(db.planWorkouts)
          ..where((t) => t.planId.equals(planId))
          ..orderBy([(t) => OrderingTerm.asc(t.position)]))
        .get();
    if (workouts.isEmpty) return null;
    final ids = workouts.map((w) => w.id).toList();
    final lastSession = await (db.select(db.workoutSessions)
          ..where((t) => t.planWorkoutId.isIn(ids) & t.finishedAt.isNotNull())
          ..orderBy([
    ...
    final lastIndex =
        workouts.indexWhere((w) => w.id == lastSession.planWorkoutId);
  ...
  Future<int> startPlanSession(PlanWorkout workout) async {
    final sessionId = await startSession(
        planWorkoutId: workout.id, title: workout.name);
    final planExercises = await (db.select(db.planExercises)
    ...
    for (var i = 0; i < planExercises.length; i++) {
      ...
      for (var s = 1; s <= pe.targetSets; s++) {
        final prev =
            last != null && last.sets.length >= s ? last.sets[s - 1] : null;
        await db.into(db.sessionSets).insert(SessionSetsCompanion.insert(
              sessionExerciseId: linkId,
              setNumber: s,
              weightKg: Value(prev?.weightKg),
...
final sessionExercisesProvider = StreamProvider.autoDispose
    .family<List<SessionExerciseData>, int>((ref, sessionId) =>
        ref.watch(trainingRepositoryProvider).watchSessionExercises(sessionId));
